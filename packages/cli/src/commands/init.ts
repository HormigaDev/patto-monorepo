...
async function fetchLatestTag(): Promise<string> {
    const response = await fetch(
        "https://api.github.com/repos/HormigaDev/patto-bot-template/releases/latest",
        {
            headers: {
    ...
    if (!response.ok) {
        throw new Error(
            `No se pudo obtener la ultima release: ${response.statusText}`,
        );
    }
...
async function downloadAndExtractZip(
    ...
    const zipPath = path.join(
        ...
        `temp-patto-${Date.now()}.zip`,
    );

    const response = await fetch(zipUrl);
    if (!response.ok) {
        throw new Error(
            `No se pudo descargar el release ${tag}: ${response.statusText}`,
        );
    }
...
export function registerInitCommand(cli: CAC): void {
    cli.command(
        "init [name]",
        "Inicializa un nuevo proyecto Patto Bot Template",
    )
        .option(
            "--name <name>",
            "Nombre del proyecto (alternativa al argumento posicional)",
        )
        ...
        .action(async (nameArg: string | undefined, options: InitOptions) => {
            ...
            const folderName = removeDiacritics(projectName).replace(
                /\s+/g,
                "",
            );
            ...
            if (existsSync(projectDir)) {
                console.error(
                    chalk.red(
                        `El directorio "${folderName}" ya existe en ${process.cwd()}.`,
                    ),
                );
                process.exitCode = 1;
                return;
            ...
            try {
                if (hasGit) {
                    spinner.start(
                        `Clonando repositorio en ${chalk.bold(folderName)}...`,
                    );
                    execSync(
                    ...
                    spinner.succeed(
                        `Release ${chalk.bold(tag)} descargada y extraida.`,
                    );
                }
                ...
                if (hasGit) {
                    ...
                    execSync(
                        'git commit -m "Initial Commit: Project Created" --no-verify',
                        {
                            cwd: projectDir,
                            stdio: "ignore",
                        },
                ...
                if (!hasGit) {
                    ...
                    console.log(
                        chalk.gray(
                            '  git init && git add . && git commit -m "Initial Commit: Project Created"',
                        ),
                    );
                }
            } catch (error) {
